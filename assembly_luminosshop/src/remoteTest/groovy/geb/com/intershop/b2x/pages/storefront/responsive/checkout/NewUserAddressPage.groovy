...
class NewUserAddressPage extends StorefrontPage
{
    ...
    static content =
    {
        ...
        stateSelector   { contentSlot.$('select',id:"billing_State") }
    ...
    // link functions
    //------------------------------------------------------------
    def fillData(country,company,fName,lName,address,city,state,postal,user)
    {
    ...
    def setAddress(Address address, String email)
    {
        waitFor { countrySelector.displayed }
        countrySelector.value(address.country)
        sleepForNSeconds(2)
        companyName.value address.company
        fNameInput.value address.firstName
        lNameInput.value address.lastName
        address1Input.value address.addressLine
        cityInput.value address.city
        stateSelector.value(address.state)
